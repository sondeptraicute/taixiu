...
$data_bc = $mysqli->query("SELECT * FROM `lognapbank` WHERE `uid` = '".$id."'   ORDER by id DESC LIMIT $start, $kmess");
$log = array();
while($phien=$data_bc->fetch_assoc())
{
   $msg.='<tr id="count"><td>'.$phien['id'].' </td><td>'.$phien['thoigian'].' </td> <td>'.$phien['telco'].'</td><td>'.$phien['amount'].'</td><td>'.number_format($phien['serial']).'</td><td>'.$phien['code'].'</td>   <td><font color="puprple">'.($phien['trangthai']==0 ? 'Chờ' : '</font>').' <font color="green">'.($phien['trangthai']==1 ? 'Thành công' : '</font>').' <font color="red">'.($phien['trangthai']==2 ? 'Hủy' : '</font>').' </td>  </tr>';
}
