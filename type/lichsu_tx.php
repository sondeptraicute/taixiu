<?PHP
$title = 'Lịch sử cược tài xỉu';
include_once('../function/head.php');
$data_bc = $mysqli->query("SELECT * FROM `cuoc_taixiu` WHERE `uid` = '".$id."'   ORDER by id DESC LIMIT 150");
$log = array();
while($phien=$data_bc->fetch_assoc())
{

   $msg.='<tr id="count"><td><font color="red"><b>#'.$phien['phien'].'</b></font> ('.$phien['thoigian'].') </td> <td>'.$phien['type'].' ('.($phien['code'] <=0 ? '<font color="blue">Chờ</font>' : '').''.($phien['code'] ==1 ? '<font color="green">Xong</font>' : '').''.($phien['code'] >=2 ? '<font color="red">T.Về</font>' : '').') </td> <td>'.number_format($phien['truoc']).' </td> <td>'.number_format($phien['xucuoc']).' </td>  <td>'.number_format($phien['hoantra']).' </td> <td>'.number_format($phien['xunhan']).' </td> <td>'.number_format($phien['sau']).' </td>  </tr>';
}
