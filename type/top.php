...
$data_bc = $mysqli->query("SELECT * FROM `nguoichoi` ORDER by xu DESC LIMIT $start, $kmess");
$log = array();
while($phien=$data_bc->fetch_assoc())
{
   $msg.='<tr id="count"><td>#'.$phien['id'].' - '.$phien['taikhoan'].' </td> <td>'.number_format($phien['xu']).' </td>    </tr>';
}
...
<h2 class="panel-title">Bảng xếp hạng</h2><br>
</header>
