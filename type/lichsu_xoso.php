...
$data_bc = $mysqli->query("SELECT * FROM `xoso` WHERE `uid` = '".$id."'   ORDER by id DESC LIMIT $start, $kmess");
$log = array();
while($phien=$data_bc->fetch_assoc())
{
   $msg.='<tr id="count"><td><font color="red"><b>#'.$phien['id'].'</b><td>'.$phien['ngay'].'</td><td>'.$phien['so'].'</td><td>'.$phien['ketqua'].'</td><td>'.number_format($phien['xu']).'</td><td>'.number_format($phien['xuwin']).'</td>   <td><font color="puprple">'.($phien['trangthai']==0 ? 'Chờ' : '</font>').' <font color="green">'.($phien['trangthai']==1 ? 'Thắng' : '</font>').' <font color="red">'.($phien['trangthai']==2 ? 'Thua' : '</font>').' </td>  </tr>';
}
...
function cuocthoigian()
{
 $.ajax({
url : '/game/send?cuocthoigian&p='+trang,
type : 'POST',
success : function(d){
if(d.het == 1) {
...
<h2 class="panel-title">Lịch sử xổ số</h2>
</header>
